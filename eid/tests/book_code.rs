use openmls::prelude::{Ciphersuite, Credential, CredentialType, CredentialWithKey, CryptoConfig, Extensions, KeyPackage, OpenMlsCryptoProvider, SignatureScheme};
...
fn book_operations(ciphersuite: Ciphersuite, backend: &impl OpenMlsCryptoProvider) {
    ...
    let alice = EidMlsMember::new((alice_key_package, alice_credential));
    ...
    let bob = EidMlsMember::new((bob_key_package, bob_credential));
    let charlie = EidMlsMember::new((charlie_key_package, charlie_credential));
    let dave = EidMlsMember::new((dave_key_package, dave_credential));
    ...
    // === Bob updates and commits ===
    // ANCHOR: self_update
    let (mls_message_out, welcome_option, _group_info) = bob_eid
    ...
    assert_eq!(
        ...
        bob_eid.export_secret(backend, "", &[], 32)
    );

    // Make sure that both groups have the same public tree
    assert_eq!(
        alice_group.export_ratchet_tree(),
        bob_eid.export_ratchet_tree()
    );
    ...
    let bob_processed_message = bob_eid
        .process_message(
            backend,
            mls_message_out
                .into_protocol_message()
        ...
        bob_processed_message.into_content()
    {
        ...
        bob_eid.store_pending_proposal(*staged_proposal);
    } else {
    ...
    let (queued_message, welcome, _group_info) = bob_eid
    ...
    let mut charlie_group = MlsGroup::new_from_welcome(
        ...
        Some(bob_eid.export_ratchet_tree()),
    )
    ...
    let _alice_processed_message = alice_group
        ...
        .expect("Could not process message.");
    let _bob_processed_message = bob_eid
        .process_message(
            backend,
            queued_message
                .into_protocol_message()
        ...
        bob_processed_message.into_content()
    {
        ...
        let remove_operation = RemoveOperation::new(remove_proposal, &bob_eid)
            .expect("An unexpected Error occurred.");
    ...
    assert!(!bob_eid.is_active());
    let members = bob_eid.members().collect::<Vec<Member>>();
